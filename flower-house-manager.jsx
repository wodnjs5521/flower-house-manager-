...
/* 60대 사용성: 작업 우선순위 라벨 (색 + 글자 함께 표시) */
const TASK_LEVELS = {
  urgent: { label: "긴급", color: "#E0533C" },
  today: { label: "오늘", color: "#E08A3C" },
  soon: { label: "예정", color: "#C9A93E" },
...
/* 한국어 받침 판별 조사 선택 (예: josa(name,'은','는')) */
...
/* 꽃 별칭 사전 (배열 첫 항목 = 화면에 보여줄 대표 이름). 검색·자동완성·판별 공용 */
const FLOWER_ALIASES = {
  장미: ["장미", "rose"],
  리시안셔스: ["리시안시스", "리시안셔스", "리시안서스", "리시안사스", "꽃도라지", "유스토마", "lisianthus", "eustoma"],
  ...
  수국: ["수국", "hydrangea"], 안개초: ["안개꽃", "안개초", "집소필라", "gypsophila"],
...
function flowerProfile(name) {
  ...
  const key = Object.keys(FLOWER_ALIASES).find((k) => FLOWER_ALIASES[k].some((a) => n.includes(a.replace(/\s/g, "").toLowerCase())));
  return { key: key || "기타", profile: FLOWER_PROFILES[key] || FLOWER_PROFILES.기타, exact: !!key };
}

/* 꽃 이름 자동완성 후보 (기본 꽃 + 사용자 추가 꽃). 일부 글자만 입력해도 좁혀짐 */
function flowerSuggestions(query) {
  const q = (query || "").replace(/\s/g, "").toLowerCase();
  const out = [];
  for (const [key, custom] of Object.entries(_activeCustomFlowers)) {
    const aliases = Array.isArray(custom.aliases) ? custom.aliases : [key];
    const hit = !q || [key, ...aliases].some((a) => String(a).replace(/\s/g, "").toLowerCase().includes(q));
    if (hit) out.push({ name: key, isCustom: true });
  }
  for (const key of FLOWER_LIST) {
    if (key === "기타") continue;
    const aliases = FLOWER_ALIASES[key] || [key];
    const matched = q ? aliases.find((a) => a.replace(/\s/g, "").toLowerCase().includes(q)) : null;
    const hit = !q || !!matched || key.replace(/\s/g, "").toLowerCase().includes(q);
    if (!hit) continue;
    const korMatched = matched && /[가-힣]/.test(matched) ? matched : null;
    const display = korMatched || aliases.find((a) => /[가-힣]/.test(a)) || key;
    out.push({ name: display, isCustom: false });
  }
  const seen = new Set();
  return out.filter((o) => { if (seen.has(o.name)) return false; seen.add(o.name); return true; }).slice(0, 8);
}
...
const PEST_DAMAGE_STAGE = {
  "굴파리": "larva", "나방": "larva", "작은뿌리파리": "larva", "뿌리혹선충": "larva",
};
function pestDamageStage(name) {
  const n = String(name || "");
  const k = Object.keys(PEST_DAMAGE_STAGE).sort((a, b) => b.length - a.length).find((key) => n.includes(key));
...
function pestLifecycle(name) {
  ...
  return k ? PEST_LIFECYCLE[k] : DEFAULT_LIFECYCLE;
}
function normalizeLifecycle(obj) {
  if (!obj) return null;
...
function computePestStage(pestName, customLcObj) {
  ...
  let acc = 0;
  const stageInfos = stages.map((s) => { const startOff = acc; acc += s.days; return { ...s, startOff, endOff: acc }; });
  const pos = daysSince - cycleBase;
  ...
  const span = current.endOff - current.startOff;
  const frac = span > 0 ? (pos - current.startOff) / span : 0;
  const nextKey = curIdx + 1 < stageInfos.length ? stageInfos[curIdx + 1].key : stageInfos[0].key;
  let danger = 0;
  if (current.key === damageKey) danger += 0.6;
  if (nextKey === damageKey && frac >= 0.5) danger += 0.35;
  if (frac >= 0.6) danger += 0.18;
  if (isAdultNow && damageKey === "adult") danger += 0.12;
  danger = Math.max(0, Math.min(1, danger));
  let actionLevel, action;
...
function computeWatering(house, profInfo, ctx) {
  ...
  const hasWater = !!lastWater;
  const prof = profInfo.profile;
  const base = prof.water || 2;
  const planting = plantingRoundsFor(house)[0]?.date;
  const today = todayISO();
  const daysSince = planting ? dayDiff(planting, today) : null;
  let stageLabel = "—", mult = 1;
  if (daysSince !== null) {
    if (daysSince < 0) stageLabel = "정식 전";
    else if (daysSince < 14) { stageLabel = "활착기"; mult = 0.7; }
    else { stageLabel = "생육기"; mult = 1; }
  }
  const w = weather || {};
  let weatherNote = "";
  if ((w.high >= 32) || (w.cur >= 30)) { mult *= 0.75; weatherNote = "고온기 · 물마름 빠름"; }
  if ((w.humidity >= 85) || (w.rainProb >= 70) || ((w.rain ?? 0) > 0)) { mult *= 1.4; weatherNote = "다습·강수 · 과습 주의"; }
  let interval = Math.round(base * mult);
  interval = Math.max(1, Math.min(base + 3, interval));
  const lastWaterDate = hasWater ? localISODate(lastWater.ts) : null;
  const nextWaterDate = lastWaterDate ? addDaysISO(lastWaterDate, interval) : today;
  const dueToday = !hasWater || dayDiff(today, nextWaterDate) <= 0;
  return { hasWater, lastWaterTs: lastWater?.ts || null, lastWaterDate, nextWaterDate, intervalDays: interval, stageLabel, weatherNote, dueToday };
}
...
function computeHouseEstimate(house, ctx) {
  ...
  return {
    ...
    vent: hs.vent,
    shade: hs.shade,
    pestForecast: computePestForecast(house, profInfo),
    watering: computeWatering(house, profInfo, ctx),
  };
}
...
/* 60대 사용성: 우선순위 라벨 칩 (색 + 글자). level: urgent/today/soon/info */
function TaskTag({ level, children }) {
  const lv = TASK_LEVELS[level] || TASK_LEVELS.info;
  return (
    <span style={{
      display: "inline-flex", alignItems: "center", gap: 5,
      background: lv.color + "22", color: lv.color, border: `1.5px solid ${lv.color}`,
      borderRadius: 999, padding: "4px 12px", fontSize: 14, fontWeight: 800, whiteSpace: "nowrap",
    }}>
...
/* 큰 핵심값 표시 (60대 사용성: 라벨 작게, 값 크게) */
function BigStat({ label, value, sub, color }) {
  return (
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 13, color: T.faint, fontWeight: 700, marginBottom: 3 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 900, color: color || T.ink, lineHeight: 1.15, wordBreak: "keep-all" }}>{value}</div>
...
/* 해충 충태 정사각형 그리드.
   한 칸 = 벌레 이름 + 현재 충태 + 그 충태가 언제부터~언제까지.
   꽃에 위협적인(가해 단계 진행/임박) 벌레일수록 진한 빨강, 약할수록 옅게.
   열 수는 화면 너비에 따라 자동(3·4칸 이상). */
function PestGrid({ pests }) {
  if (!pests || !pests.length) {
    return <div style={{ fontSize: 13.5, color: T.faint, lineHeight: 1.6 }}>꽃 이름과 정식일을 등록하면 취약 해충의 충태가 표시됩니다.</div>;
  }
  return (
...
/* 꽃 이름 자동완성 입력 (기본 꽃 + 사용자 추가 꽃, '추가' 배지) */
function FlowerAutocomplete({ value, onChange, placeholder }) {
  const [q, setQ] = useState(value || "");
  const [open, setOpen] = useState(false);
  useEffect(() => { setQ(value || ""); }, [value]);
  const sugg = flowerSuggestions(q);
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {open && sugg.length > 0 && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 30, background: T.panel,
          border: `1px solid ${T.line}`, borderRadius: 12, marginTop: 4, overflow: "hidden", boxShadow: "0 8px 24px rgba(0,0,0,.35)" }}>
          {sugg.map((s, i) => (
            <button key={s.name + i} type="button" onMouseDown={(ev) => ev.preventDefault()} onClick={() => pick(s.name)}
              style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", textAlign: "left", padding: "13px 14px",
                background: "transparent", border: "none", borderBottom: i < sugg.length - 1 ? `1px solid ${T.line}55` : "none",
                color: T.ink, fontSize: 16, cursor: "pointer" }}>
              <span style={{ fontWeight: 700 }}>{s.name}</span>
              {s.isCustom && <span style={{ background: T.accent, color: T.accentInk, borderRadius: 6, fontSize: 11, padding: "2px 7px", fontWeight: 800 }}>추가</span>}
            </button>
          ))}
        </div>
      )}
...
function Root() {
  ...
  // ---- 개폐기 자동화: 1분마다 + 날씨 변경 + 설정 변경 시 즉시 재계산 ----
  const [ventTick, setVentTick] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setVentTick((n) => n + 1), 60000);
    return () => clearInterval(t);
  }, []);
  const enabledAutoCount = Object.values(state.ventAutoConfigs || {}).filter((c) => c?.enabled).length;
  useEffect(() => {
    if (!farm) return;
  ...
  }, [ventTick, weather.cur, weather.rainProb, weather.rain, farm?.id, enabledAutoCount]);
  ...
  return (
    <div style={shell}>
      ...
      {detailHouseId ? (
        <>
          <BackLayer onClose={() => setDetailHouseId(null)} />
          <HouseDetail
            {...screenProps}
        ...
        <>
          {tab !== "today" && <BackLayer onClose={() => setTab("today")} />}
...
function AuthScreen({ state, update, showToast }) {
  ...
  const normLogin = (s) => (s || "").trim().toLowerCase();
  const doSignup = async () => {
    if (!form.name || !form.login || !form.pwd) return showToast("이름·아이디·비밀번호를 입력하세요");
    const loginKey = normLogin(form.login).replace(/\//g, "-");
    // 중복 확인: 로컬 + 클라우드 양쪽 모두
    if (state.users.some((u) => normLogin(u.login).replace(/\//g, "-") === loginKey)) return showToast("이미 있는 아이디입니다");
  ...
  const doLogin = async () => {
    if (loggingIn) return;
    const loginNorm = normLogin(form.login).replace(/\//g, "-");
    let u = state.users.find((x) => normLogin(x.login).replace(/\//g, "-") === loginNorm);
    let fromCloud = false;
    let farmSyncCode = null;

    // 로컬에 없으면 클라우드에서 조회 (loginNorm = loginKey)
    if (!u && sync.isConfigured()) {
      setLoggingIn(true);
      showToast("계정 확인 중…");
      const cloudUser = await sync.fetchUser(loginNorm);
      setLoggingIn(false);
      if (cloudUser) {
    ...
    if (!u) {
      if (!sync.isConfigured()) return showToast("이 기기에 계정이 없습니다. 처음 가입한 기기에서 로그인하거나 인터넷 연결을 확인하세요");
      return showToast("아이디를 찾을 수 없습니다. 아이디를 확인하거나 회원가입 후 이용하세요");
    }
    if (!(await verifyPwd(form.pwd, u.pwd))) return showToast("비밀번호가 맞지 않습니다");

    // 로컬 로그인 성공 시 클라우드에 없으면 저장 (기존 가입자 자동 마이그레이션)
    if (!fromCloud && sync.isConfigured()) {
      sync.fetchUser(loginNorm).then(async (existing) => {
        if (!existing) {
          const myFarmForCloud = state.farms.find((f) => f.ownerId === u.id);
          await sync.saveUser(loginNorm, {
            id: u.id, name: u.name, login: u.login, pwdHash: u.pwd,
            farmSyncCode: myFarmForCloud?.syncCode || null, createdAt: localISODate(),
          });
        }
      }).catch(() => {});
    }
...
function TodayScreen({ state, farm, setTab, setDetailHouseId, update, weather, weatherLoading, weatherError }) {
  ...
  // 하우스별 오늘 우선도 (긴급/오늘/예정/참고)
  const houseLevel = (est) => {
    if (!est.active) return "info";
    if (est.status === "danger" || est.pestForecast?.actionLevel === "urgent") return "urgent";
    if (est.watering?.dueToday || est.status === "caution" || est.pestForecast?.actionLevel === "soon") return "today";
    return "info";
  };
  const activeRows = rows.filter((r) => r.est.active);
  const leveledRows = activeRows.map((r) => ({ ...r, level: houseLevel(r.est) }));
  const order = { urgent: 0, today: 1, soon: 2, info: 3 };
  leveledRows.sort((a, b) => (order[a.level] - order[b.level]) || (b.est.score - a.est.score));
  const urgentCount = leveledRows.filter((r) => r.level === "urgent").length;
  const generalCount = leveledRows.length - urgentCount;
  const inactiveRows = rows.filter((r) => !r.est.active);

  // 다가오는 일 (3·7·14일)
  const upcoming = { d3: [], d7: [], d14: [] };
  activeRows.forEach((r) => {
    const pf = r.est.pestForecast, wt = r.est.watering, bl = r.est.bloomEstimate;
    const push = (date, text, level) => {
      if (!date) return;
      const diff = dayDiff(todayISO(), date);
      if (diff <= 0 || diff > 14) return;
      const bucket = diff <= 3 ? "d3" : diff <= 7 ? "d7" : "d14";
      upcoming[bucket].push({ house: r.house.name, text, level, diff, date });
    };
    if (pf && pf.daysToAdult > 0) push(pf.adultDate, `${pf.pest} 성충 전환 예상`, pf.daysToAdult <= 3 ? "urgent" : "soon");
    if (wt && !wt.dueToday) push(wt.nextWaterDate, "물주기 예정", "info");
    if (bl) push(bl.centerDate, `${bl.round}차 개화 예상`, "soon");
  });
  Object.values(upcoming).forEach((arr) => arr.sort((a, b) => a.diff - b.diff));
  const hasUpcoming = upcoming.d3.length + upcoming.d7.length + upcoming.d14.length > 0;

  return (
    <div>
      <ScreenHeader title="오늘 할 일" sub={dateLabel} />

      {/* 큰 요약: 긴급 / 일반 */}
      <div style={{ padding: "0 14px 8px" }}>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => setSummaryHelp({ key: "urgent", label: "긴급", n: urgentCount, suffix: "개",
              description: "지금 먼저 살펴보는 것이 좋은 하우스입니다.",
              criteria: "병해충 성충 전환 임박, 위험 점수 높음, 오늘 물주기 필요 중 하나라도 해당되면 긴급입니다.",
              matches: leveledRows.filter((r) => r.level === "urgent").map((r) => r.house.name) })}
            style={{ flex: 1, background: TASK_LEVELS.urgent.color + "1A", border: `2px solid ${TASK_LEVELS.urgent.color}`, borderRadius: 18, padding: "16px 12px", cursor: "pointer", textAlign: "center" }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: TASK_LEVELS.urgent.color }}>긴급</div>
            <div style={{ fontSize: 40, fontWeight: 900, color: TASK_LEVELS.urgent.color, lineHeight: 1.1 }}>{urgentCount}</div>
            <div style={{ fontSize: 13, color: T.sub, fontWeight: 700 }}>개 하우스</div>
          </button>
          <button onClick={() => setSummaryHelp({ key: "general", label: "일반", n: generalCount, suffix: "개",
              description: "오늘 큰 위험은 없지만 상태를 확인하면 좋은 하우스입니다.",
              criteria: "긴급으로 분류되지 않은 재배 중 하우스입니다.",
              matches: leveledRows.filter((r) => r.level !== "urgent").map((r) => r.house.name) })}
            style={{ flex: 1, background: T.panel, border: `2px solid ${T.line}`, borderRadius: 18, padding: "16px 12px", cursor: "pointer", textAlign: "center" }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: T.sub }}>일반</div>
            <div style={{ fontSize: 40, fontWeight: 900, color: T.ink, lineHeight: 1.1 }}>{generalCount}</div>
            <div style={{ fontSize: 13, color: T.sub, fontWeight: 700 }}>개 하우스</div>
          </button>
        </div>
        {/* 보조 요약 칩 (절화·농약·출하) */}
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
          {summaryItems.filter((it) => ["cut", "pest", "shipment"].includes(it.key)).map((it) => (
            <button key={it.key} onClick={() => setSummaryHelp(it)}
              style={{ flex: 1, minWidth: 90, background: T.panel2, border: `1px solid ${T.line}`, borderRadius: 12, padding: "10px 8px", cursor: "pointer", textAlign: "center" }}>
              <div style={{ fontSize: 13, color: T.sub, fontWeight: 700 }}>{it.label}</div>
              <div style={{ fontSize: 20, fontWeight: 900, color: it.n > 0 ? T.ink : T.faint }}>{it.n}<span style={{ fontSize: 12, fontWeight: 700, color: T.faint }}>{it.suffix || "건"}</span></div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "6px 14px 4px" }}>
        <Card>
          ...
          {/* 시간별 24시간 예보 스트립 */}
          {weather.source === "live" && weather.next24 && weather.next24.length > 0 && (
            <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${T.line}` }}>
              <div style={{ fontSize: 12, color: T.faint, marginBottom: 6, fontWeight: 700 }}>시간별 날씨 (24시간)</div>
              <div style={{
                display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4,
                scrollbarWidth: "none", WebkitOverflowScrolling: "touch",
              }}>
                {weather.next24.map((h, i) => (
                  <div key={i} style={{
                    minWidth: 52, flexShrink: 0, textAlign: "center",
                    background: i === 0 ? T.accent + "22" : T.bg2 || T.line + "44",
                    border: i === 0 ? `1.5px solid ${T.accent}` : `1px solid ${T.line}`,
                    borderRadius: 10, padding: "6px 4px",
                  }}>
                    <div style={{ fontSize: 11, color: i === 0 ? T.accent : T.faint, fontWeight: i === 0 ? 800 : 600, marginBottom: 2 }}>{h.label}</div>
                    <div style={{ fontSize: 17 }}>{h.emoji}</div>
                    <div style={{ fontSize: 13, fontWeight: 800, marginTop: 2 }}>{h.temp}°</div>
                    {h.rainProb >= 20 && (
                      <div style={{ fontSize: 10, color: "#5B9BD5", fontWeight: 700, marginTop: 1 }}>{h.rainProb}%</div>
                    )}
                  </div>
                ))}
              </div>
      ...
      <div style={{ padding: "10px 14px 4px" }}>
        ...
        {leveledRows.map((r) => {
          const e = r.est, pf = e.pestForecast, wt = e.watering;
          const lv = TASK_LEVELS[r.level];
          return (
            <Card key={r.house.id} style={{ marginBottom: 12, padding: 16, borderLeft: `6px solid ${lv.color}` }}>
              {/* 헤더: 하우스명 + 꽃 + 우선도 */}
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontSize: 22, fontWeight: 900, lineHeight: 1.15 }}>{r.house.name}</div>
                  <div style={{ fontSize: 15, color: T.sub, fontWeight: 700 }}>{r.house.flower || "꽃 미등록"}</div>
                </div>
                <TaskTag level={r.level} />
              </div>

              {/* 핵심값 3개 */}
              <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
                <BigStat label="다음 물주기"
                  value={wt.dueToday ? "오늘 필요" : fmtDate(wt.nextWaterDate)}
                  sub={wt.lastWaterDate ? `마지막 ${fmtDate(wt.lastWaterDate)}` : "물 기록 없음"}
                  color={wt.dueToday ? TASK_LEVELS.today.color : T.ink} />
                <BigStat label="현재 충태"
                  value={pf ? pf.stageLabel : "—"}
                  sub={pf ? pf.pest : "꽃·정식일 등록 필요"}
                  color={pf && pf.actionLevel === "urgent" ? TASK_LEVELS.urgent.color : T.ink} />
                <BigStat label="성충 예상"
                  value={pf ? fmtDate(pf.adultDate) : "—"}
                  sub={pf ? (pf.isAdultNow ? "현재 성충기" : `${pf.daysToAdult}일 뒤`) : ""}
                  color={pf && pf.daysToAdult <= 3 ? TASK_LEVELS.urgent.color : T.ink} />
              </div>

              {/* 오늘 조치 */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                {wt.dueToday && <TaskTag level="today">물주기</TaskTag>}
                {pf && <TaskTag level={pf.actionLevel}>{pf.action}</TaskTag>}
                {!wt.dueToday && !pf && <span style={{ fontSize: 14, color: T.sub }}>오늘 급한 작업 없음 · 상태 확인</span>}
              </div>

              {/* 개화 + 상세보기 */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "space-between" }}>
                <span style={{ fontSize: 13.5, color: e.bloomEstimate ? "#EE9AC2" : T.faint, fontWeight: 700 }}>
                  🌸 {e.bloomEstimate ? `${e.bloomEstimate.round}차 개화 ${fmtDate(e.bloomEstimate.centerDate)}` : "개화 예상 —"}
                </span>
                <button onClick={() => setDetailHouseId(r.house.id)}
                  style={{ background: T.accent, color: T.accentInk, border: "none", borderRadius: 12, padding: "12px 18px", fontSize: 15, fontWeight: 800, cursor: "pointer" }}>
                  상세보기 ›
                </button>
              </div>
            </Card>
        ...
        {leveledRows.length === 0 && (
          <Card><Empty text={rows.length === 0 ? "아직 하우스가 없습니다. 설정에서 하우스를 추가하세요." : "꽃과 정식일을 등록하면 오늘 할 일이 표시됩니다."} /></Card>
        )}
      </div>
      ...
      {inactiveRows.length > 0 && (
        <div style={{ padding: "6px 14px 4px" }}>
          <SectionTitle>설정 필요</SectionTitle>
          {inactiveRows.map((r) => (
            <Card key={r.house.id} onClick={() => setDetailHouseId(r.house.id)} style={{ marginBottom: 8, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ width: 12, height: 12, borderRadius: "50%", border: `1.5px dashed ${T.faint}`, display: "inline-block" }} />
              <div style={{ fontWeight: 800, fontSize: 16 }}>{r.house.name}</div>
              <div style={{ marginLeft: "auto", fontSize: 13.5, color: T.faint, fontWeight: 700 }}>꽃·정식일 등록 ›</div>
            </Card>
          ))}
        </div>
      )}
      ...
      {hasUpcoming && (
        <div style={{ padding: "12px 14px 4px" }}>
          <SectionTitle>다가오는 일</SectionTitle>
          {[["d3", "3일 이내"], ["d7", "7일 이내"], ["d14", "14일 이내"]].map(([k, label]) => (
            upcoming[k].length > 0 && (
              <Card key={k} style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 15, fontWeight: 900, color: T.accent, marginBottom: 10 }}>{label}</div>
                {upcoming[k].map((u, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < upcoming[k].length - 1 ? `1px solid ${T.line}55` : "none" }}>
                    <TaskTag level={u.level}>{fmtDate(u.date)}</TaskTag>
                    <span style={{ fontWeight: 800, fontSize: 15 }}>{u.house}</span>
                    <span style={{ fontSize: 14, color: T.sub, flex: 1, wordBreak: "keep-all" }}>{u.text}</span>
                  </div>
                ))}
              </Card>
            )
...
/* ============================ 개폐기 자동화 =========================== */
// 비 판정: 실제 강수량 > 0  OR  강수확률 70% 이상 (습도와 무관)
function computeAutoVentState(cfg, weather) {
  if (!cfg?.enabled) return null;
  const isRaining = (weather?.rain ?? 0) > 0 || (weather?.rainProb ?? 0) >= 70;
  if (cfg.rainClose && isRaining) return "closed";
  const now = new Date();
...
function QuickScreen({ state, update, farm, showToast, setDetailHouseId, weather }) {
  ...
  const recordWork = (btn) => {
    ...
    if (btn.key === "photo") {
      const photoHouseId = sel[sel.length - 1];
      if (!photoHouseId) return showToast("하우스를 먼저 선택하세요");
      if (sel.length > 1) showToast(`가장 최근 선택 하우스로 사진을 기록합니다`);
      return setPhotoModal({ houseId: photoHouseId });
    }
...
function MemoModal({ update, farm, houses, targetHouses, onClose, showToast }) {
  ...
  return (
    <Modal onClose={onClose} title="특이사항 기록" isDirty={() => !!memo.trim()} onSave={save}>
...
/* ====== 안드로이드 하드웨어 뒤로가기 → 앱 내 뒤로가기(레이어 닫기) ======
   열린 레이어(모달·상세화면)를 history 항목과 1:1로 맞춘다. 하드웨어 뒤로가기(또는 iOS 스와이프
   뒤로가기)가 들어오면 가장 위 레이어를 닫고, 입력 중(dirty)이면 닫기 전에 저장 확인을 띄운다.
   레이어가 없으면 정상 뒤로가기(앱/탭 종료)로 둔다. */
const _backLayers = [];
let _backSuppress = false;
if (typeof window !== "undefined" && !window.__fhmBackInit) {
  window.__fhmBackInit = true;
  window.addEventListener("popstate", () => {
    if (_backSuppress) { _backSuppress = false; return; }
    const top = _backLayers[_backLayers.length - 1];
    if (!top) return; // 닫을 레이어 없음 → 정상 뒤로가기
    try { window.history.pushState({ fhmLayer: true }, ""); } catch (e) {} // 소비된 항목 복구(상위 보호)
    if (top.isDirty()) top.requestConfirm();
    else top.close();
  });
}
function useBackClose(onClose, isDirty, requestConfirm) {
  const ref = useRef({});
  ref.current.close = onClose;
  ref.current.isDirty = isDirty;
  ref.current.requestConfirm = requestConfirm;
  useEffect(() => {
    const layer = {
      close: () => { if (ref.current.close) ref.current.close(); },
      isDirty: () => !!(ref.current.isDirty && ref.current.isDirty()),
      requestConfirm: () => { if (ref.current.requestConfirm) ref.current.requestConfirm(); },
    };
    _backLayers.push(layer);
    try { window.history.pushState({ fhmLayer: true }, ""); } catch (e) {}
    return () => {
      const i = _backLayers.indexOf(layer);
      if (i >= 0) _backLayers.splice(i, 1);
      _backSuppress = true;
      try { window.history.back(); } catch (e) {}
      // 안전장치: popstate가 _backSuppress를 먼저 소비하므로 이 타이머는 그보다 늦게 둔다(경쟁 방지)
      setTimeout(() => { _backSuppress = false; }, 400);
    };
  }, []);
}
/* 화면(모달 아님) 레이어용: 하드웨어 뒤로가기로 닫기. UI는 렌더하지 않음 */
function BackLayer({ onClose }) {
  useBackClose(onClose, () => false, () => {});
  return null;
}

function Modal({ children, onClose, title, isDirty, onSave }) {
  const [confirm, setConfirm] = useState(false);
  const dirty = () => !!(isDirty && isDirty());
  const attemptClose = () => { if (dirty()) setConfirm(true); else onClose(); };
  useBackClose(onClose, dirty, () => setConfirm(true));
  return (
    <div onClick={attemptClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 460, background: T.panel, borderRadius: "20px 20px 0 0", padding: "18px 18px calc(24px + env(safe-area-inset-bottom))", maxHeight: "86vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 16 }}>
          ...
          <button onClick={attemptClose} aria-label="닫기" style={{ flexShrink: 0, width: 44, height: 44, borderRadius: 12, background: T.panel2, border: `1px solid ${T.line}`, color: T.sub, fontSize: 22, fontWeight: 800, cursor: "pointer", lineHeight: 1 }}>✕</button>
        </div>
        {children}
      </div>
      {confirm && (
        <div onClick={(e) => e.stopPropagation()} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 320, display: "flex", alignItems: "center", justifyContent: "center", padding: 22 }}>
          <div style={{ width: "100%", maxWidth: 360, background: T.panel, border: `1px solid ${T.line}`, borderRadius: 18, padding: 20 }}>
            <div style={{ fontSize: 19, fontWeight: 900, marginBottom: 8 }}>{onSave ? "저장하시겠습니까?" : "나가시겠습니까?"}</div>
            <div style={{ fontSize: 14.5, color: T.sub, lineHeight: 1.6, marginBottom: 18 }}>
              입력한 내용이 있습니다. 저장하지 않고 나가면 사라집니다.
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {onSave && <Btn kind="primary" onClick={() => { setConfirm(false); onSave(); }} style={{ width: "100%" }}>저장하고 나가기</Btn>}
              <Btn kind={onSave ? "ghost" : "danger"} onClick={() => { setConfirm(false); onClose(); }} style={{ width: "100%" }}>{onSave ? "저장 안 함" : "나가기"}</Btn>
              <Btn kind="plain" onClick={() => setConfirm(false)} style={{ width: "100%" }}>취소(계속 입력)</Btn>
            </div>
          </div>
        </div>
      )}
...
function HarvestModal({ house, est, update, user, farm, onClose, showToast }) {
  ...
  return (
    <Modal onClose={onClose} title={`${house.name} · 오늘 꽃 땄음`} isDirty={() => !!amount || pct !== startPct} onSave={save}>
      {round > 1 && (
      ...
      <div style={{ padding: "0 14px" }}>
        ...
        <Card style={{ marginBottom: 10 }}>
          ...
          {e.pestForecast ? (
            <>
              <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
                <BigStat label="주의 해충" value={e.pestForecast.pest} />
                <BigStat label="현재 충태" value={e.pestForecast.stageLabel}
                  color={e.pestForecast.actionLevel === "urgent" ? TASK_LEVELS.urgent.color : T.ink} />
                <BigStat label="성충 예상" value={fmtDate(e.pestForecast.adultDate)}
                  sub={e.pestForecast.isAdultNow ? "현재 성충기" : `${e.pestForecast.daysToAdult}일 뒤`}
                  color={e.pestForecast.daysToAdult <= 3 ? TASK_LEVELS.urgent.color : T.ink} />
              </div>
              {/* 충태 흐름 타임라인 */}
              <div style={{ display: "flex", gap: 4, marginBottom: 12 }}>
                {e.pestForecast.stageDates.map((s, i) => {
                  const isCur = s.key === e.pestForecast.stageKey;
                  return (
                    <div key={i} style={{ flex: 1, textAlign: "center", background: isCur ? T.accent + "22" : T.panel2,
                      border: `1.5px solid ${isCur ? T.accent : T.line}`, borderRadius: 10, padding: "8px 4px" }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: isCur ? T.accent : (s.past ? T.sub : T.faint) }}>{s.label}</div>
                      <div style={{ fontSize: 12.5, color: T.sub, marginTop: 3 }}>{fmtDate(s.date)}경</div>
                      {isCur && <div style={{ fontSize: 10.5, color: T.accent, fontWeight: 800, marginTop: 2 }}>지금</div>}
                    </div>
                  );
                })}
              </div>
              <div style={{ marginBottom: 10 }}><TaskTag level={e.pestForecast.actionLevel}>{e.pestForecast.action}</TaskTag></div>
              <div style={{ background: T.panel2, borderRadius: 10, padding: 12, fontSize: 13.5, color: T.sub, lineHeight: 1.7 }}>
                {house.flower || "이 꽃"}의 주요 해충인 <b style={{ color: T.ink }}>{e.pestForecast.pest}</b>{josa(e.pestForecast.pest, "은", "는")} 현재 <b style={{ color: T.ink }}>{e.pestForecast.stageLabel}</b> 상태로 예상됩니다.
                생애주기 흐름상 {e.pestForecast.stageDates.map((s) => `${fmtDate(s.date)}경 ${s.label}`).join(", ")} 순으로 진행되며,
                {" "}<b style={{ color: T.ink }}>{fmtDate(e.pestForecast.adultDate)}경</b> 성충 전환이 예상됩니다.
                성충 전환 전후에는 산란·확산 가능성이 있어 예찰 또는 예방 방제를 준비하는 것이 좋습니다.
                <br /><span style={{ color: T.faint }}>{e.pestForecast.note}</span>
            ...
            <div style={{ fontSize: 13.5, color: T.faint, lineHeight: 1.6 }}>
              꽃 이름과 정식일을 등록하면 해당 꽃의 취약 해충 충태(알·유충·번데기·성충)를 오늘 기준으로 자동 예측합니다.
            </div>
          )}
        </Card>
        ...
        {/* 물주기 (참고용) */}
        <Card style={{ marginBottom: 10 }}>
          <SectionTitle>물주기</SectionTitle>
          <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
            <BigStat label="다음 물주기"
              value={e.watering.dueToday ? "오늘 필요" : fmtDate(e.watering.nextWaterDate)}
              sub={`${e.watering.stageLabel} · ${e.watering.intervalDays}일 간격`}
              color={e.watering.dueToday ? TASK_LEVELS.today.color : T.ink} />
            <BigStat label="마지막 물 준 날"
              value={e.watering.lastWaterDate ? fmtDate(e.watering.lastWaterDate) : "기록 없음"}
              sub={e.watering.lastWaterTs ? daysAgoLabel(e.watering.lastWaterTs) : "물주기 기록 시작"} />
          </div>
          {e.watering.weatherNote && (
            <div style={{ marginBottom: 10 }}><TaskTag level="soon">{e.watering.weatherNote}</TaskTag></div>
          )}
          <div style={{ background: T.panel2, borderRadius: 10, padding: 12, fontSize: 13.5, color: T.sub, lineHeight: 1.7 }}>
            마지막으로 물 준 날과 현재 생육 단계({e.watering.stageLabel}), 날씨를 반영한 예상입니다.
            {e.watering.stageLabel === "활착기" && " 막 심은 초기에는 활착을 위해 물 상태를 자주 확인하세요."}
            {e.watering.stageLabel === "생육기" && " 어느 정도 자란 단계로 초기보다 간격을 조금 늘릴 수 있습니다."}
            <br /><span style={{ color: T.faint, fontSize: 12 }}>※ 참고용 안내입니다. 실제 물주기는 하우스 내부 흙 상태·습도·온도·작물 상태를 보고 최종 판단하세요.</span>
          </div>
        </Card>
        ...
        {(() => {
          ...
          const updateVentCfg = (patch) => update((s) => {
            const newCfg = { ...ventCfg, ...patch };
            const newConfigs = { ...(s.ventAutoConfigs || {}), [houseId]: newCfg };
            if (newCfg.enabled) {
              const autoState = computeAutoVentState(newCfg, weather);
              if (autoState) {
                return { ...s, ventAutoConfigs: newConfigs, houseState: { ...s.houseState, [houseId]: { ...(s.houseState[houseId] || {}), vent: autoState, updatedAt: Date.now() } } };
              }
            }
            return { ...s, ventAutoConfigs: newConfigs };
          });
              ...
              {ventCfg.enabled ? (
                <>
                  <div style={{ background: T.panel2, borderRadius: 10, padding: "10px 12px", marginBottom: 12 }}>
                    ...
                    {ventCfg.rainClose && ((weather.rain ?? 0) > 0 || weather.rainProb >= 70) && (
                      <div style={{ color: T.caution, fontSize: 12.5, marginTop: 4 }}>{(weather.rain ?? 0) > 0 ? `실제 강수 ${weather.rain}mm` : `강수확률 ${weather.rainProb}%`} → 비 우선 닫힘</div>
                    )}
                  </div>
                  ...
                  <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", marginBottom: 12 }}>
                    ...
                    <span style={{ fontSize: 14, color: T.ink }}>비 올 때 닫기 (실제 강수 또는 강수확률 70% 이상)</span>
                  </label>
...
function HouseEditModal({ house, update, onClose, showToast, farm, user, onDeleted }) {
  ...
  useEffect(() => { if (_initRef.current === null) _initRef.current = JSON.stringify(f); }, []);
  const isDirty = () => _initRef.current !== null && JSON.stringify(f) !== _initRef.current;
  ...
  return (
    <Modal onClose={onClose} title={`${house.name} 정보`} isDirty={isDirty} onSave={save}>
      <Field label="하우스 이름"><input style={inputStyle} value={f.name} onChange={set("name")} /></Field>

      <Field label="꽃 이름 (몇 글자만 입력하면 후보가 나옵니다)">
        <FlowerAutocomplete value={f.flower || ""} onChange={(v) => setF((x) => ({ ...x, flower: v }))} placeholder="예: 안 → 안개꽃, 리 → 리시안시스" />
        {/* 꽃 이미지: 자동 검색 + 직접 추가 */}
...
function WorkLogEditModal({ log, houseName, update, onClose, showToast, onViewHouse }) {
  ...
  const _initRef = useRef(null);
  useEffect(() => { if (_initRef.current === null) _initRef.current = JSON.stringify({ type, detail, quantity, memo, when }); }, []);
  const isDirty = () => _initRef.current !== null && JSON.stringify({ type, detail, quantity, memo, when }) !== _initRef.current;
  ...
  return (
    <Modal onClose={onClose} title={`${houseName} · 기록 수정`} isDirty={isDirty} onSave={save}>
...

{
  ...
  "prevention": {
    ...
    "pestLifecycles": {
      "해충명1": { "egg": 4, "larva": 7, "pupa": 5, "adult": 12 }
    }
  }
}
...
- pestLifecycles: 각 해충의 25℃ 전후 생애주기 일수. 알(egg)·유충 또는 약충(larva)·번데기(pupa)·성충(adult)이 되기까지 걸리는 일수를 정수로. 응애·진딧물처럼 번데기가 없으면 pupa에 약충 기간을 넣어도 됩니다. 앱이 심은 날짜 기준으로 오늘 충태를 추정하는 데 사용합니다. pests의 첫 번째 해충은 반드시 포함하세요.`;
  ...
  const doImport = () => {
    ...
    const entry = {
      ...
      prevention: parsed.prevention ? {
        ...
        pestLifecycles: (parsed.prevention.pestLifecycles && typeof parsed.prevention.pestLifecycles === "object") ? parsed.prevention.pestLifecycles : {},
      } : null,
