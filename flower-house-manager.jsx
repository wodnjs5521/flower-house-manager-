...
function computePestForecast(house, profInfo) {
  ...
  const prevention = getFlowerPrevention(profInfo.key) || {};
  const pests = (prevention.pests || []).filter(Boolean);
  if (!pests.length) return null;
  const pestName = pests[0];
  const customLc = normalizeLifecycle(_activeCustomFlowers[profInfo.key]?.prevention?.pestLifecycles?.[pestName]);
  const lc = customLc || pestLifecycle(pestName);
  ...
  const current = stageInfos.find((s) => pos >= s.startOff && pos < s.endOff) || stageInfos[stageInfos.length - 1];
  ...
  return { pest: pestName, allPests: pests, stageLabel: current.label, stageKey: current.key, stageDates, adultDate, daysToAdult, isAdultNow, actionLevel, action, note: lc.note };
}
